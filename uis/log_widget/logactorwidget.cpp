...
#include "ui_logactorwidget.h"

LogActorWidget::LogActorWidget(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::LogActorWidget)
{
    ui->setupUi(this);
...
void LogActorWidget::refresh()
{
    QList<Log> logs = LogData::getIns()->getLog(LOG_ACTOR);
    refreshUI(logs);
}

void LogActorWidget::refreshUI(const QList<Log> logs)
{
...
void LogActorWidget::slotSearch()
{
    ...
    QList<Log> logs = LogData::getIns()->getLog(LOG_ACTOR,dt_begin, dt_end);

    refreshUI(logs);
}

void LogActorWidget::slotExport()
{
    //
}
